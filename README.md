...
**A 4-tier model escalation chain** guards against truncated or degenerate output: `default model (2 attempts) → claude-haiku-4.5 (2 attempts) → a top-tier model (2 attempts) → a last-resort model (1 attempt)`, escalating only once every attempt at the current tier has also failed. Three independent failure signals trigger escalation, not just content quality: `finish_reason === 'length'` (hit the token cap), a heuristic for a response that finished on its own but degenerated into a long, punctuation-less run-on, and a plain HTTP failure from a fallback tier's own model (e.g. a removed/invalid model id) — the last of these skips straight to the next tier rather than retrying a broken model id, since a real incident (a deprecated tier-2 model returning a 404) showed the chain used to fail the whole call outright on this signal instead of escalating past it. Every discarded attempt gets logged as its own real row (model, tokens, cost, outcome) the moment it's decided, not batched at the end — the call log shows the fallback happening, not just the final result. A live-in-progress card shows the model actually being attempted right now, updating as the chain escalates.
