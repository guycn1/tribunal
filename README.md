...
Transient failures are split by **how long they took**, because the right response differs: a call that bounces back in under 10 seconds (a burst rate limit, say) gets a bounded number of free same-model retries that don't spend a tier attempt — escalating to a pricier model within seconds of a rate limit that clears on its own would be exactly the wrong move — while one that burns its whole ceiling is treated as a real failure of that tier and escalates. Per-attempt timeouts scale with both prompt size and the token cap, so a judge's much larger prompt gets a proportionally larger ceiling.
